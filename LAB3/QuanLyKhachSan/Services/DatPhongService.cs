...
namespace QuanLyKhachSan.Services
{
    ...
    public class DatPhongService
    {
        ...
        public (bool ThanhCong, string ThongBao) ThemKhach(string ma, string ten, string cmnd, string qt, string sdt)
        {
            if (string.IsNullOrWhiteSpace(ma) || string.IsNullOrWhiteSpace(ten) || string.IsNullOrWhiteSpace(cmnd) || string.IsNullOrWhiteSpace(qt))
                return (false, "Thông tin khách chưa đầy đủ.");
            try
            {
        ...
        public (bool ThanhCong, string ThongBao) TaoDatPhong(string so, string maKhach, string maNV, DateTime ngayLap, DateTime nhan, DateTime tra, decimal coc, string kenh, List<PhongDatItem> ds)
        {
            if (string.IsNullOrWhiteSpace(so) || string.IsNullOrWhiteSpace(maKhach) || string.IsNullOrWhiteSpace(maNV) || ds == null || ds.Count == 0)
                return (false, "Phiếu đặt phòng chưa đủ thông tin.");
            if (tra.Date < nhan.Date)
                return (false, "Ngày trả dự kiến không được trước ngày nhận.");
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    foreach (var x in ds)
                    {
                        ...
                        if (o == null) return (false, "Không tìm thấy phòng " + x.SoPhong);
                        if (x.SoNguoi <= 0 || x.SoNguoi > Convert.ToInt32(o))
                            return (false, "Số người của phòng " + x.SoPhong + " vượt sức chứa.");
                        if (PhongTrungLich(cn, tx, x.SoPhong, nhan, tra))
                            return (false, "Phòng " + x.SoPhong + " bị trùng lịch đặt.");
                    }
                    ...
                    tx.Commit();
                    return (true, "Đã lập phiếu đặt phòng.");
                }
                catch (Exception ex)
                {
        ...
        public (bool ThanhCong, string ThongBao) ThemNguoiLuuTru(string so, string phong, string ten, string cmnd, string qt)
        {
            if (string.IsNullOrWhiteSpace(so) || string.IsNullOrWhiteSpace(phong) || string.IsNullOrWhiteSpace(ten) || string.IsNullOrWhiteSpace(cmnd) || string.IsNullOrWhiteSpace(qt))
                return (false, "Thông tin người lưu trú chưa đầy đủ.");
            try
            {
                ...
                if (dem >= max) return (false, "Đã đủ số người đăng ký cho phòng này.");
                ...
                Db.Execute("INSERT INTO NguoiLuuTru(SoPhieuDat, SoPhong, HoTen, SoCMND, QuocTich) VALUES(@s, @p, @t, @c, @q)",
                    ...
                    new SqlParameter("@c", cmnd),
                    new SqlParameter("@q", qt));
                return (true, "Đã thêm người lưu trú.");
            }
            catch (Exception ex)
            {
                return (false, ex.Message);
        ...
        public (bool ThanhCong, string ThongBao) NhanPhong(string so, DateTime thucTe)
        {
            using (var cn = Db.OpenConnection())
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    c.Parameters.AddWithValue("@s", so);
                    if (c.ExecuteNonQuery() == 0) return (false, "Phiếu không ở trạng thái có thể nhận phòng.");
        ...
        public (bool ThanhCong, string ThongBao) DanhDauNoShow(string so)
        {
            try
            {
                ...
                return (true, "Đã đánh dấu không nhận phòng (No-show).");
            }
            catch (Exception ex)
            {
                return (false, ex.Message);
